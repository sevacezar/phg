...
def get_unified_data_per_well_without_interpolation(model_names, historical_df,
    ...
    # 4. Выводим информацию о загруженных параметры
...
def save_to_excel_structured_single_sheet(well_dataframes, historical_df, models_data, 
    ...
    1 строка: well, date, wbp_hist, wbp_hist_smoothed, wbp_hist_smoothed_max, wbp_hist_smoothed_min, model_name1, "", "", model_name2, "", "", ...
                ...
                if date_key in smoothed_pressures[well_name]:
                    ...
                    for dict_date_key in smoothed_pressures[well_name].keys():
                        try:
                            ...
                            if str(dict_date_key) == date_key:
                                hist_smoothed = smoothed_pressures[well_name][dict_date_key]
                                break
                        except:
                            continue
            
            # Получаем максимумы и минимумы для этой даты
    ...
    # Красный цвет для максимумов (столбец E)
    red_fill = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")
    
    # Синий цвет для минимумов (столбец F)
    ...
    try:
        ...
        # Проверяем, есть ли экстремумы в файле
        print(f"\n  Проверка экстремумов в файле:")
        maxima_count = 0
        minima_count = 0
        
        for row in range(4, min(ws.max_row + 1, 50)):  # Проверяем первые 50 строк данных
            max_cell = ws[f'E{row}']
            min_cell = ws[f'F{row}']
            
            if max_cell.value is not None:
                maxima_count += 1
                if maxima_count <= 5:  # Выводим первые 5 максимумов
                    well_name = ws[f'A{row}'].value
                    date_val = ws[f'B{row}'].value
                    print(f"    Максимум строка {row}: {well_name}, {date_val} -> {max_cell.value:.2f}")
            
            if min_cell.value is not None:
                minima_count += 1
                if minima_count <= 5:  # Выводим первые 5 минимумов
                    well_name = ws[f'A{row}'].value
                    date_val = ws[f'B{row}'].value
                    print(f"    Минимум строка {row}: {well_name}, {date_val} -> {min_cell.value:.2f}")
        
        print(f"    Всего максимумов в первых {min(ws.max_row - 3, 50)} строках: {maxima_count}")
        print(f"    Всего минимумов в первых {min(ws.max_row - 3, 50)} строках: {minima_count}")
