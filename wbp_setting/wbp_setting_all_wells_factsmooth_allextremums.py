# Скрипт для анализа качества настройки пластового давления по скважинам. Можно сопоставлять факт сразу с несколькими загруженными моделями
# В качестве входных данных - текстовый файл с фактическими давлениями по формату "Скважина	Дата(ДД.ММ.ГГГГ)	Давления(в барах)"
# В качестве выходных данных - иксэль файл с результатми сопоставления, а также архив с графиками.
# Важно! Для построения графиков - нужно чтобы работал сервер (по умолчанию комп Севы) и переменная CREATE_GRAPHS скрипта была True
# Чтобы запустить сервер с компа Севы, нужно: 1) Зайти в VS Code 2) Открыть проект C:\Users\V_Kryazhev\VSCode\easy_server 3) в терминале выполнить команду для активации виртуального окружения: ".venv/Scripts/activate" 4) в терминале выполнить команду для запуска сервера: "python main.py" 5) Если в выводе терминала есть "INFO:     Uvicorn running on http://0.0.0.0:5000 (Press CTRL+C to quit)" - сервер запущен.
...
WBP_FACT_TXT: str = "wbp_adapt/Pfkt0.inc"  # Относительный путь (относительно DATA-файла модели) до файла с фактическими давлениями
...
EXCLUDE_END_DAYS: int = 70  # Количество дней с конца периода, в которых экстремумы будут исключены (для корректного сопоставления факта и моделей)
...
# ============================================================================
# ОПИСАНИЕ РАСЧЕТА МЕТРИК КАЧЕСТВА СООТВЕТСТВИЯ МОДЕЛИ И ФАКТА
# ============================================================================
# Метрики качества соответствия рассчитываются на основе данных из листа "Экстремумы"
# и сохраняются на листе "Качество соответствия". Ниже подробно описано, как рассчитать
# каждую метрику вручную на основе данных листа "Экстремумы".
#
# ИСХОДНЫЕ ДАННЫЕ:
# - На листе "Экстремумы" для каждой скважины хранятся последние 5 максимумов и 5 минимумов
#   для факта (model_name = "FACT") и для каждой модели
# - Экстремумы отсортированы по дате в убывающем порядке (от новых к старым)
# - extremum_order = 1 соответствует самому последнему (самому новому) экстремуму
# - extremum_order = 2 соответствует предпоследнему экстремуму и т.д.
#
# ПРИНЦИП СОПОСТАВЛЕНИЯ:
# - Последний максимум факта (extremum_order=1, extremum_type=max) сопоставляется
#   с последним максимумом модели (extremum_order=1, extremum_type=max)
# - Предпоследний максимум факта (extremum_order=2) сопоставляется с предпоследним
#   максимумом модели (extremum_order=2) и т.д.
# - Аналогично для минимумов: последний минимум факта сопоставляется с последним
#   минимумом модели и т.д.
#
# МЕТРИКА 1: phase_deviation_days (отклонение по фазе, дни)
# Описание: Среднее отклонение по фазе - показывает, на сколько дней модель опережает
#           или отстает от факта по времени наступления экстремумов
# Расчет:
#   1. Для каждой пары сопоставленных максимумов (факт и модель с одинаковым extremum_order):
#      phase_diff_max = дата_максимума_модели - дата_максимума_факта (в днях)
#   2. Для каждой пары сопоставленных минимумов:
#      phase_diff_min = дата_минимума_модели - дата_минимума_факта (в днях)
#   3. phase_deviation_days = среднее_арифметическое(всех phase_diff_max и phase_diff_min)
# Значение:
#   - Положительное: модель опережает факт (экстремумы в модели наступают раньше)
#   - Отрицательное: модель отстает от факта (экстремумы в модели наступают позже)
#   - Ноль: идеальное совпадение по фазе
# Пример:
#   Факт: максимум 1 на 2024-01-15, максимум 2 на 2023-07-10
#   Модель: максимум 1 на 2024-01-20, максимум 2 на 2023-07-05
#   phase_diff_max1 = 2024-01-20 - 2024-01-15 = +5 дней
#   phase_diff_max2 = 2023-07-05 - 2023-07-10 = -5 дней
#   phase_deviation_days = (5 + (-5)) / 2 = 0 дней
#
# МЕТРИКА 2: amplitude_deviation (отклонение амплитуды, бар)
# Описание: Средняя разность амплитуд - показывает, насколько амплитуда колебаний
#           давления в модели отличается от амплитуды в факте
# Расчет:
#   1. Для каждой пары сопоставленных экстремумов (максимум и минимум с одинаковым extremum_order):
#      амплитуда_факта = значение_максимума_факта - значение_минимума_факта
#      амплитуда_модели = значение_максимума_модели - значение_минимума_модели
#      amplitude_diff = амплитуда_модели - амплитуда_факта
#   2. amplitude_deviation = среднее_арифметическое(всех amplitude_diff)
# Значение:
#   - Положительное: амплитуда в модели больше, чем в факте (более сильные колебания)
#   - Отрицательное: амплитуда в модели меньше, чем в факте (более слабые колебания)
#   - Ноль: идеальное совпадение амплитуд
# Пример:
#   Факт: максимум 1 = 100 бар, минимум 1 = 80 бар
#   Модель: максимум 1 = 105 бар, минимум 1 = 75 бар
#   амплитуда_факта = 100 - 80 = 20 бар
#   амплитуда_модели = 105 - 75 = 30 бар
#   amplitude_diff = 30 - 20 = +10 бар
#   amplitude_deviation = +10 бар (если это единственная пара)
#
# МЕТРИКА 3: max_deviation (отклонение максимумов, бар)
# Описание: Средняя разность абсолютных значений максимумов - показывает, насколько
#           максимальные значения давления в модели отличаются от факта
# Расчет:
#   1. Для каждой пары сопоставленных максимумов (с одинаковым extremum_order):
#      max_diff = значение_максимума_модели - значение_максимума_факта
#   2. max_deviation = среднее_арифметическое(всех max_diff)
# Значение:
#   - Положительное: максимумы в модели выше, чем в факте (завышенные пики давления)
#   - Отрицательное: максимумы в модели ниже, чем в факте (заниженные пики давления)
#   - Ноль: идеальное совпадение максимумов
# Пример:
#   Факт: максимум 1 = 100 бар, максимум 2 = 95 бар
#   Модель: максимум 1 = 105 бар, максимум 2 = 92 бар
#   max_diff1 = 105 - 100 = +5 бар
#   max_diff2 = 92 - 95 = -3 бар
#   max_deviation = (5 + (-3)) / 2 = +1 бар
#
# МЕТРИКА 4: min_deviation (отклонение минимумов, бар)
# Описание: Средняя разность абсолютных значений минимумов - показывает, насколько
#           минимальные значения давления в модели отличаются от факта
# Расчет:
#   1. Для каждой пары сопоставленных минимумов (с одинаковым extremum_order):
#      min_diff = значение_минимума_модели - значение_минимума_факта
#   2. min_deviation = среднее_арифметическое(всех min_diff)
# Значение:
#   - Положительное: минимумы в модели выше, чем в факте (завышенные минимумы давления)
#   - Отрицательное: минимумы в модели ниже, чем в факте (заниженные минимумы давления)
#   - Ноль: идеальное совпадение минимумов
# Пример:
#   Факт: минимум 1 = 80 бар, минимум 2 = 75 бар
#   Модель: минимум 1 = 78 бар, минимум 2 = 73 бар
#   min_diff1 = 78 - 80 = -2 бар
#   min_diff2 = 73 - 75 = -2 бар
#   min_deviation = (-2 + (-2)) / 2 = -2 бар
#
# ВАЖНО:
# - Все метрики сохраняют знак (не используются абсолютные значения), чтобы можно было
#   понять направление отклонения модели от факта
# - Если для какой-то пары экстремумов нет данных (например, в модели меньше экстремумов,
#   чем в факте), то эта пара не учитывается в расчете
# - Метрики рассчитываются отдельно для каждой модели и каждой скважины
# - На листе "Качество соответствия" каждая строка содержит метрики для одной скважины
#   и одной модели
# ============================================================================
